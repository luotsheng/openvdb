...
function ActionNode(props: NodeProps<Data>) {
    const { data, selected } = props;

    const getHandleClass = (port: Port) => {
        if (port.type === 'exec') {
            return 'rf-handle-exec';   // 执行引脚：白色/粗
        }
        // 数据引脚按类型着色
    ...
    return (
        <div className={`rf-node action-node ${selected ? "selected" : ""}`}>
            <div className="rf-node-header action-header">
                {data.title}
            </div>
